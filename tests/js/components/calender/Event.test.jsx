...
describe('Test the Event Component of Calender', () => {
	...
	const count = 0;
	const getCalenderData=jest.fn()
	const wrapper = shallow(
		<Event calender={calender} getCalenderData={getCalenderData} />
	);
	...
	it('test the getCalenderData function',()=>{
		expect(getCalenderData.mock.calls.length).toBe(1)
	})
	it('test the Object.keys',()=>{
		expect(Object.keys(calender).length).toBe(2);
	})
	it('test the Object.keys.counter',()=>{
		expect(calender.counter).toBe(0);
	})
	it('test the Object.keys.counter',()=>{
		calender=''
		expect(calender).toBe('');
	})
});
